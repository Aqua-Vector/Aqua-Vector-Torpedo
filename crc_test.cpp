...
uint8_t calculate_crc8(const uint8_t* buf, size_t len) {
    uint8_t check = 0;
    while(len--) {
...
int main() {
    ...
    std::cout << "CRC8 (Excluding Sync): 0x" << std::hex << (int)calculate_crc8(data, 14) << std::endl;
    ...
    std::cout << "CRC8 (Including Sync): 0x" << std::hex << (int)calculate_crc8(data_with_sync, 16) << std::endl;

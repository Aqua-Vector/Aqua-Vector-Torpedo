...
public:
    ...
    size_t serialize(const UplinkPacket& pkt, uint8_t* buf, size_t max_len) {
        ...
        // UplinkPacket은 이미 sync(0xBB), payload, crc16을 멤버로 가지고 있음
...
int main() {
    ...
    NetworkManager<LegacyGcsParser, UartLink, UplinkPacket> gcs_nm(gcs_link, gcs_parser, gcs_tx_q);
    ...
    TorpedoControlSystem tcs(mux, am, manual_source, auto_source, imu, eskf, gcs_nm, stm_nm);
